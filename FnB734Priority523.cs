...
#region Implementation
    ...
    foreach(var _ in ReadCase())
    {
      ...
      if (AsChar(entities.Case1.Status) == 'O')
      {
        if (ReadInfrastructure())
        {
          ...
          if (ReadServiceProviderCaseAssignment1())
          {
      ...
      else if (ReadServiceProviderCaseAssignment2())
      {
        local.DashboardAuditData.WorkerId = entities.ServiceProvider.UserId;
      }
  ...
  private void CreateDashboardStagingPriority35()
  {
    ...
    var param = 0M;
    var caseClosures = local.DashboardStagingPriority35.CaseClosures ?? 0;
  ...
  private IEnumerable<bool> ReadCase()
  {
    return ReadEachInSeparateTransaction("ReadCase",
      (db, command) =>
      {
        ...
        db.SetNullableDate(command, "statusDate", import.ReportStartDate.Date);
  ...
  private bool ReadServiceProviderCaseAssignment2()
  {
    ...
    return Read("ReadServiceProviderCaseAssignment2",
      ...
      {
        db.SetString(command, "casNo", entities.Case1.Number);
        db.SetDate(command, "effectiveDate", entities.Case1.StatusDate);
      },
      (db, reader) =>
      {
        entities.ServiceProvider.SystemGeneratedId = db.GetInt32(reader, 0);
  ...
  private void UpdateDashboardStagingPriority2()
  {
    var asOfDate = local.DashboardStagingPriority35.AsOfDate;
    var caseClosures =
      (entities.DashboardStagingPriority35.CaseClosures ?? 0) + 1;
