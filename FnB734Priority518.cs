  ...
  public void Run()
  {
    ...
    foreach(var _ in ReadCashReceiptDetail1())
    {
      ...
      local.DashboardStagingPriority35.DaysToIwoPaymentDenominator =
        (local.DashboardStagingPriority35.DaysToIwoPaymentDenominator ?? 0) + 1
        ;
      local.DashboardStagingPriority35.DaysToIwoPaymentNumerator =
        (local.DashboardAuditData.DaysReported ?? 0) + (
          local.DashboardStagingPriority35.DaysToIwoPaymentNumerator ?? 0);

      if ((local.DashboardStagingPriority35.DaysToIwoPaymentDenominator ?? 0) <=
        0 || (
          local.DashboardStagingPriority35.DaysToIwoPaymentNumerator ?? 0) <= 0
        )
      {
      ...
      else
      {
        local.DashboardStagingPriority35.DaysToIwoPaymentAverage =
          (decimal)(local.DashboardStagingPriority35.
            DaysToIwoPaymentNumerator ?? 0) / (
            local.DashboardStagingPriority35.DaysToIwoPaymentDenominator ?? 0);
      }
...
ReadEach:
  ...
  private void CreateDashboardStagingPriority35()
  {
    ...
    var daysToIwoPaymentAverage =
      local.DashboardStagingPriority35.DaysToIwoPaymentAverage ?? 0M;
  ...
  private bool ReadFipsTribunal()
  {
    ...
    return Read("ReadFipsTribunal",
      (db, command) =>
      {
        db.SetInt32(command, "identifier", entities.LegalAction.TrbId ?? 0);
      },
      (db, reader) =>
      {
