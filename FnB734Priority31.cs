...
#region Implementation
    ...
    foreach(var _ in ReadLegalReferralCase())
    {
      ...
      if (!IsEmpty(local.DashboardAuditData.JudicialDistrict))
      {
        ...
        local.Local1.Update.G.CasesWithEnfReferral =
          (local.Local1.Item.G.CasesWithEnfReferral ?? 0) + (
            local.DashboardStagingPriority35.CasesWithEnfReferral ?? 0);
        local.Local1.Update.G.CasesWithEstReferral =
          (local.Local1.Item.G.CasesWithEstReferral ?? 0) + (
            local.DashboardStagingPriority35.CasesWithEstReferral ?? 0);
      }
        ...
        import.ProgramCheckpointRestart.UpdateFrequencyCount ?? 0))
      {
        ...
        local.ProgramCheckpointRestart.RestartInfo =
          Substring(local.ProgramCheckpointRestart.RestartInfo, 250, 1, 99) + String
          (local.BatchTimestampWorkArea.TextTimestamp,
  ...
  private void CreateDashboardStagingPriority35()
  {
    ...
    var casesWithEstReferral = local.Local1.Item.G.CasesWithEstReferral ?? 0;
    var casesWithEnfReferral = local.Local1.Item.G.CasesWithEnfReferral ?? 0;
    var param = 0M;
  ...
  private IEnumerable<bool> ReadLegalReferralCase()
  {
    return ReadEachInSeparateTransaction("ReadLegalReferralCase",
      (db, command) =>
      {
        ...
        db.SetDateTime(command, "createdTimestamp", local.End.Timestamp);
