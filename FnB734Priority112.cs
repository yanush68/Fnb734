      ...
      foreach(var _1 in ReadOfficeOfficeStaffing())
      {
        if (entities.OfficeStaffing.Populated)
        {
          local.Local1.Update.GofficeStaffing.FullTimeEquivalent =
            (local.Local1.Item.GofficeStaffing.FullTimeEquivalent ?? 0M) + (
...
Test:
      ...
      if ((local.Temp.CollectionsPerFteDenominator ?? 0M) == 0)
      {
      ...
      else
      {
        local.Temp.CollectionsPerFteAverage =
          Math.Round((local.Temp.CollectionsPerFteNumerator ?? 0M) /
          (local.Temp.CollectionsPerFteDenominator ?? 0M), 2,
    ...
    foreach(var _ in ReadDashboardStagingPriority2())
    {
      ...
      if ((entities.DashboardStagingPriority12.CollectionsPerFteAverage ?? 0M) ==
        (local.PrevRank.CollectionsPerFteAverage ?? 0M))
      {
  ...
  {
    var collectionsPerFteNumerator = local.Temp.CollectionsPerFteNumerator ?? 0M
      ;
    var collectionsPerFteDenominator =
      local.Temp.CollectionsPerFteDenominator ?? 0M;
    var collectionsPerFteAverage = local.Temp.CollectionsPerFteAverage ?? 0M;
  ...
  {
    var collectionsPerFteRank = local.Temp.CollectionsPerFteRank ?? 0;
